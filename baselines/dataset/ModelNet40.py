...
def load_data(data_root, partition='train'):
    ...
    elif partition == 'dis_attack':
        return npz['test_pc'], npz['test_label'], npz['target_label'], npz['ori_pc']
    else:
...
class ModelNet40Hybrid(ModelNet40):
    ...
    def __init__(self, ori_data, def_data, num_points,
        ...
        if partition == 'test':
            ori_data = ori_data[:,:num_points,:]
        else:
            ori_data = random_sample_points_np(ori_data, num_points)
        def_data, def_label = load_data(def_data, partition='test')
        def_data = def_data[..., :3]
...
class ModelNet40dis_attack(Dataset):
    """Modelnet40 dataset for target attack evaluation.
    We return an additional target label for an example.
    """

    def __init__(self, data_root, num_points, normalize=True):
        self.data, self.label, self.target,self.orig_pc = \
            load_data(data_root, partition='dis_attack')
        self.num_points = num_points
        self.normalize = normalize
    ...
    def __getitem__(self, item):
        ...
        label = self.label[item]
        target = self.target[item]
        orig_pc=self.orig_pc[item]
        if self.normalize:
            pc = normalize_points_np(pc)

        return pc, label, target ,orig_pc
