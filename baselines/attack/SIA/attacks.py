...
class PointCloudAttack(object):
    ...
    def load_models(self, classifier, model_name):
        ...
        """
        model_path = os.path.join('../checkpoint/' + 'ModelNet40', model_name)
    ...
    def shape_invariant_ifgm(self, points, target):
        ...
        for i in range(self.max_steps):
            ...
            if not self.defense_method is None:
                logits = self.wb_classifier(self.pre_head(points))[0]
            else:
                logits = self.wb_classifier(points)[0]
        ...
        with torch.no_grad():
            ...
            if not self.defense_method is None:
                adv_logits = self.classifier(self.pre_head(points.transpose(1, 2).detach()))[0]
            else:
                adv_logits = self.classifier(points.transpose(1, 2).detach())[0]
            if adv_logits.size(0)==1:
                adv_target = adv_logits.data.max(1)[1]
            else:
                adv_logits=adv_logits.unsqueeze(0)
                adv_target = adv_logits.data.max(1)[1]
