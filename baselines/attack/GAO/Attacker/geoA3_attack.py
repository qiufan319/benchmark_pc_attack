...
def _forward_step(net, pc_ori, input_curr_iter, normal_ori, ori_kappa, target, scale_const, cfg, targeted):
    ...
    if cfg.cls_loss_type == 'Margin':
        ...
        if targeted:
            cls_loss = nn.CrossEntropyLoss(reduction='none').cuda()(output_curr_iter[0], Variable(target, requires_grad=False))
        else:
    ...
    if cfg.dis_loss_type == 'CD':
        ...
        distance_loss=dis_loss.mean().item()
    ...
    if cfg.hd_loss_weight !=0:
        ...
        distance_loss=distance_loss+hd_loss.mean().item()
    else:
        hd_loss = 0
    ...
    if cfg.curv_loss_weight !=0:
        ...
        distance_loss = distance_loss + curv_loss.mean().item()
    else:
    ...
    return output_curr_iter, normal_curr_iter, loss, loss_n, cls_loss, dis_loss, hd_loss, curv_loss, constrain_loss, info,distance_loss
...
def attack(net, input_data, cfg, i, loader_len, saved_dir=None):
    ...
    for search_step in range(cfg.binary_max_steps):
        ...
        for step in range(cfg.iter_max_steps):
            ...
            _, normal_curr_iter, loss, loss_n, cls_loss, dis_loss, hd_loss, nor_loss, constrain_loss, info,distance_loss = _forward_step(net, pc_ori, input_curr_iter, normal_ori, kappa_ori, target, scale_const, cfg, targeted)
