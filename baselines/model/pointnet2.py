...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            norm = torch.sum((1.0 / dists),dim=2,keepdim=True)
            norm = norm.repeat(1, 1, 3)
            weight = (1.0/dists) / norm # [B, N, 3]
            weight = weight / \
...
class PointNet2ClsSsg_ai(nn.Module):
    def __init__(self, num_classes=40):
        super(PointNet2ClsSsg_ai, self).__init__()
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=3, mlp=[64, 64, 128],
                                          group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256],
                                          group_all=False)
    ...
    def forward(self, xyz):
        ...
        x = l3_points.view(B, 1024)
        x = self.drop1(F.relu(self.bn1(self.fc1(x))))
        feats = self.drop2(F.relu(self.bn2(self.fc2(x))))
        x = self.drop2(F.relu(self.bn2(self.fc2(x))))
        x = self.fc3(x)
