...
sys.path.append(os.path.join(up_path, 'baselines'))
import datetime
import logging
import provider
import importlib
...
def parse_args():
    ...
    parser.add_argument('--angles', type=float, default=1, help='random rotation bound')
...
def test(classifier, loader, rotation_pool, num_class):
    ...
    for j, (points,target,label) in tqdm(enumerate(loader), total=len(loader)):
...
def main(args):
    ...
    trainDataLoader = DataLoader(train_set, batch_size=args.batch_size,
                              shuffle=False, num_workers=0,
                              pin_memory=True, drop_last=False)
    testDataLoader = DataLoader(test_set, batch_size=args.batch_size,
                             shuffle=False, num_workers=0,
                             pin_memory=True, drop_last=False)
    '''TRAIN MODEL LOADING'''
    ...
    pretrained_dir = Path('../baselines/pretrain/pointnet_cls.pth')
    ...
    # pretrained_dir = Path('./pretrained_models/modelnet40/pointnet2_cls_msg.pth')
    # checkpoint_pre = torch.load(pretrained_dir)
    ...
    # criterion3 = cal_loss()
    pretrained_dir = Path('../baselines/pretrain/dgcnn.t7')
    ...
    #for classifier_eval in [classifier_eval1]:
        for batch_id_max, (points,target,label) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
